...
async def _run(args: argparse.Namespace) -> int:
    ...
    # Extra chromium args for media permissions (wake word / mic access) and secure origin treatment
    default_browser_args = [
        "--use-fake-ui-for-media-stream",
        "--use-fake-device-for-media-stream",
        "--unsafely-treat-insecure-origin-as-secure=http://192.168.0.109:8888,http://172.17.0.1:5173,http://localhost:8888,http://localhost:3001,http://localhost:5173,http://127.0.0.1:8888,http://127.0.0.1:3001,http://127.0.0.1:5173",
    ]
    env_extra_args = os.environ.get("BROWSER_USE_EXTRA_ARGS", "").strip()
    if env_extra_args:
        extra_chromium_args = [arg.strip() for arg in env_extra_args.split() if arg.strip()]
    else:
        extra_chromium_args = default_browser_args

    browser = None
    try:
        from browser_use import BrowserConfig
        browser_config_kwargs: dict = {
            "headless": headless,
        ...
        try:
            browser_kwargs["extra_chromium_args"] = extra_chromium_args
            browser = Browser(**browser_kwargs)
        except Exception:
            browser_kwargs.pop("extra_chromium_args", None)
            browser = Browser(**browser_kwargs)
