...
def test_output_schema():
    ...
    if basis_secondary is not None:
        assert basis_secondary in BASIS_PRIMARY, "basis_secondary must use a supported bucket"
    ...
    site_url = data.get("task_site_url")
    assert isinstance(site_url, str) and site_url.startswith("https://example.com"), (
        "task_site_url must start with https://example.com"
    )
    more_info_url = data.get("task_more_info_url")
    assert isinstance(more_info_url, str) and more_info_url.startswith("http"), (
        "task_more_info_url must be a valid URL"
    )
    ...
    screenshot_path = OUTPUT.parent / "screenshot.png"
    if screenshot_path.is_file():
        source_artifacts["screenshot"] = str(screenshot_path)
    ...
    decision_facets = [
        ...
        {
            "key": "task_more_info_url",
            "label": "More info URL",
            "role": "evidence",
            "kind": "textual",
            "value": more_info_url.strip(),
        },
    ]
    if basis_secondary is not None:
        ...
        },
        {
            ...
            "facets": [
                ...
                },
                {
                    ...
                    "value": f"The persona used a {exploration_style.replace('_', ' ')} browsing pattern to evaluate Example Domain.",
                },
            ],
        },
    ]
